...
AbstractStringAppender::AbstractStringAppender()
    : m_format(
          QLatin1String("%{time}{yyyy-MM-ddTHH:mm:ss.zzz} [%{type:-7}] <%{function}> %{message}\n"))
{}
 ...
 * \sa setFormat(const QString&)
 */
QString AbstractStringAppender::formattedString(const QDateTime &timeStamp,
    ...
    while (i < f.size()) {
        ...
        if (c != QLatin1Char(formattingMarker) || (i + 2) >= size) {
            ...
            // Thread ID (duplicates Qt5 threadid debbuging way)
                ...
                chunk = QLatin1String("0x")
                        + QString::number(qlonglong(QThread::currentThread()->currentThread()), 16);
